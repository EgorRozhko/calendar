...
$res = mysqli_query($connection,'SELECT id, date_event,Month(date_event) FROM events WHERE Year(date_event)='.$_POST['year']);
   $my_array = array();
    while($row = mysqli_fetch_array($res))
    {
        $my_array[$row['date_event']] = $row['id'];
    }
...
function draw_calendar($month, $year, $action) {
    ...
    for($list_day = 1; $list_day <= $days_in_month; $list_day++) {
        ...
        {
            $calendar.= '<div class="b-calendar__number b-calendar__weekend" id='.$action[$day].'>'.$list_day.'</div>';
            $calendar.= '</td>';
        }
        else {
